...
/// <summary>
/// The framebuffer behind a display session: applies damage rectangles, keeps the whole current
/// surface for <see cref="IDisplaySession.SnapshotAsync"/>, and turns each one into the
/// <see cref="DisplayUpdate"/> consumers see.
/// </summary>
/// <remarks>
/// <para>
...
public sealed class DisplaySurface
{
    ...
    public DisplaySurface(int dpi = 96) => _dpi = dpi;

    /// <summary>The current surface size. Zero until the first scanout.</summary>
    ...
    public DisplayUpdate ApplyScanout(
        int width, int height, int stride, DisplayPixelFormat format, byte[] pixels, DateTimeOffset at, bool mayAdoptPixels = false)
    {
        ...
        lock (_lock)
        {
            ...
            {
                Geometry = new DisplayGeometry(width, height, _dpi);
                _frame = new byte[width * height * 4];
            }

            return ApplyLocked(0, 0, width, height, stride, format, pixels, at, mayAdoptPixels)
    ...
    public DisplayUpdate? ApplyUpdate(
        int x, int y, int width, int height, int stride, DisplayPixelFormat format, byte[] pixels, DateTimeOffset at, bool mayAdoptPixels = false)
    {
        ArgumentNullException.ThrowIfNull(pixels);
        lock (_lock)
        {
            return ApplyLocked(x, y, width, height, stride, format, pixels, at, mayAdoptPixels);
        }
    }
    ...
    private DisplayUpdate? ApplyLocked(
        int x, int y, int width, int height, int stride, DisplayPixelFormat format, byte[] pixels, DateTimeOffset at, bool mayAdoptPixels)
    {
        var packedStride = width * 4;
        if (width <= 0 || height <= 0 || stride < packedStride || Geometry.Width == 0)
        {
            return null;
        }
        ...
        return new DisplayUpdate(x, y, width, height, packedStride, format, packed, Interlocked.Increment(ref _sequence), at);
    }
}
