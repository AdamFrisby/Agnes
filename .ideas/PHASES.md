...
1. `[~]` `connectivity/01-relay-and-tunneling.md` — **Mostly built:** the host transport abstraction (from AC13.6), a self-hostable **`Agnes.Relay`** blind broker (per-host-key auth, brute-force edge, Dockerfile), a **`TailscaleTransportProvider`** (tailnet-only default), and the **relay + client transports** (host blind-pumps relay data-connections to its loopback TLS Kestrel; client validates a pinned host cert; per-device auth unchanged in the tunnel) — a prompt round-trips through an in-process relay in tests (AC2/AC4/AC5/AC6). **Remaining:** real-CA certs (LettuceEncrypt relay endpoint + DNS-01 via DynDNS/BYO for the NAT'd host; Tailscale auto-certs cover the rest) and the live cross-network smoke on a relay VPS. Maintainer decisions: blind pipe, self-host-only, per-host key, lean on Tailscale.
...
2. `[ ]` `connectivity/03-session-handoff.md` — **Not built (now unblocked).** The relay transport exists; handoff reuses forking/replay (done) + a relay host-to-host path. Buildable; live verification wants the relay VPS.
3. `[ ]` `connectivity/02-multi-server-support.md` — **Not built (now unblocked).** The client already has a multi-host pool; each host is now reachable via the relay/Tailscale transport. Small; buildable.
4. `[ ]` `connectivity/04-device-linking-and-restore.md` — **Not built (now unblocked).** Reuses the relay `TransportEndpoint` (fixes the LAN-address QR bug) + `AuthFlowKind` buckets over the already-pluginized auth methods. Buildable.
...
3. `[ ]` `connectivity/05-multi-machine-workspace-model.md` — **Not built (now unblocked).** Reuses deep-git worktrees (done) + session handoff (buildable now the relay exists). Sequence after /03.
