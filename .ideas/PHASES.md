...
**Tally of the 42 scheduled items: 38 `[x]` complete · 2 `[~]` basic/partial · 2 `[—]` descoped · 0 `[ ]` not built.** `platform/04` remains excluded. Every scheduled spec is now built, partial, or descoped — nothing is unbuilt.

The **2 `[~]`** are each gated on something outside build effort:
- `sessions/04` subagent control/routing — genuinely capability-gated on the coding CLIs (none expose subagent addressing/stop yet); ships as a read-only roster with disabled controls + tooltip, ready to enable when a CLI supports it.
- `connectivity/01` relay — **code-complete** (relay binary + Tailscale + relay/client transports + certs, all offline-tested); only the live cross-network NAT-traversal smoke + real Let's Encrypt issuance remain, which need a relay VPS.

The **2 `[—]`** are maintainer descopes: `security/01` E2E (TLS deemed sufficient — the relay is a blind TLS-passthrough pipe) and `delight/01` pets (its ambient-status core is covered by the inbox + tray).

Several built features are code-complete but await a real credential/service to run *live* (voice/OpenAI, push/FCM, bridges, GitHub OAuth, Postgres, OIDC/embeddings endpoints) — all tracked in `docs/live-verification-gaps.md`.
...
3. `[x]` `sessions/03-pending-queue-and-steering.md` — queue + three send policies + send-now + discarded list, **plus true in-turn steering**: `ISteerableSession` on PTY/wrapped-CLI sessions injects via an escape-then-message primitive (`InterruptAndSend` steers when possible, cleanly falls back to cancel-then-resend for non-input-controlled ACP sessions). *(Steering completed via the maintainer's ESC-then-message technique.)*
...
4. `[x]` `sessions/07-local-cli-wrapper-and-handoff.md` — a new `agnes` wrapper binary spawns a coding CLI in a real PTY (`Porta.Pty` — already transitive via Iciclecreek; `/dev/ptmx` present), tees PTY↔console, and captures the stream as `TerminalOutputEvent`s into a normal, handoff-capable (Replay) Agnes session — reusing direct-vs-synced capture + connectivity/03 handoff. The interactive-attach keybindings are the additive outer loop. *(Also filled the missing real host-side `ICliFallback` (`PortaPtyCliFallback`), giving the embedded terminal a real PTY backend.)*
...
2. `[x]` `ops/02-memory-search.md` — the SQLite FTS5 full-text tier shipped first; the **embedding-backed semantic tier** now shipped too, config-gated behind `Agnes:Search:Embeddings:Provider=openai|local` (`none`/unset = FTS5-only, unchanged). Both providers go through Microsoft.Extensions.AI's provider-neutral `IEmbeddingGenerator` (`.OpenAI` connector → OpenAI or a local OpenAI-compatible base URL like Ollama/LM Studio); a per-chunk vector lands in a sibling `event_vectors` table in the same SQLite file (no external vector DB), a query ranks by cosine similarity, and semantic + keyword results are blended via reciprocal-rank fusion. Cosine/fusion/hybrid all offline-tested with a fake `IEmbeddingGenerator`; the real OpenAI/Ollama round-trip is the one live gap (`docs/live-verification-gaps.md` §7).
3. `[x]` `security/02-enterprise-auth.md` — OIDC token validation + mTLS client-cert + GitHub org/team gating, **plus the interactive OIDC authorization-code + PKCE redirect flow** (`/auth/oidc/start`+`/callback`, discovery, CSRF/nonce, reusing the validation core to mint the device token). Real-IdP browser round-trip is a documented live gap. *(Redirect flow completed.)*
...
4. `[x]` `ops/03-deployment-topology-and-multi-db.md` — an optional **Postgres event store** (Npgsql) behind the neutral `IEventStore`, config-selected (`Agnes:Storage:EventStore = sqlite` default `| postgres`); per-store selection so other durable stores can gain a Postgres backing the same way; storage-topology docs. SQLite default unchanged. The real Postgres round-trip is a documented live gap (skipped without a server). *(Built per the SQLite-default + optional-Postgres decision.)*
