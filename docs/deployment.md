...
| `Security:AllowUsbPassthrough` | Whether a project may pass host **USB devices** through to its sandbox VMs (a phone on adb, a serial board). **Default false.** The guest gets the raw device and the host loses it while a session holds it; a fork never inherits one. See [security.md](security.md#usb-passthrough-agnessecurityallowusbpassthrough). |
