...
| `Agnes.Agents.Antigravity` | Google Antigravity (`agy`) over `Agnes.Agents.Native`. No ACP, but a real persistent stream-json mode — one process, one NDJSON line per turn. Autonomous-only: without `--dangerously-skip-permissions` it does not ask, it writes to a scratch dir and reports success. See `docs/antigravity.md`. |
